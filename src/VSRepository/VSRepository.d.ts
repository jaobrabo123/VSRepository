...
/**
 * Detecta se uma string possui o separador 'AND' mais de uma vez.
 */
type HasMultipleANDs<S extends string> = string extends S 
    ? false 
    : S extends `${string}AND${string}AND${string}` 
        ? true 
        : false;
...
type ExtractOrs<T, S extends string, I> = S extends ''
    ? []
    : ExtractOrsTuple<T, Split<S, 'Or'>, I>;

type ExtractANDsTuple<T, Arr extends string[], I> = Arr extends [
    infer First extends string,
    ...infer Rest extends string[],
]
    ? [...ExtractOrs<T, First, I>, ...ExtractANDsTuple<T, Rest, I>]
    : [];

type ExtractFields<T, R extends string, I> = R extends ''
    ? []
    : ExtractANDsTuple<T, Split<R, 'AND'>, I>;
    ...
    methods?: {
        [K in keyof (Config extends { methods: infer Meth } ? Meth : {})]: K extends string
            ? HasMultipleANDs<K> extends true
                // Se tem múltiplos 'AND', exige proxyTo obrigatoriamente
                ? MethodConfig<M, Config extends { selectModels: infer SM } ? SM : any> & {
                      proxyTo: ValidMethodPatterns;
                  }
                // Se tiver 1 ou 0 'AND', segue a regra original (valida padrão mágico)
